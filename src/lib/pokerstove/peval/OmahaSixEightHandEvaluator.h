 ...
 * $Id: OmahaSixEightHandEvaluator.h 2649 2012-06-30 04:53:24Z prock $
 */
#ifndef PEVAL_OMAHASIXEIGHTHANDEVALUATOR_H_
#define PEVAL_OMAHASIXEIGHTHANDEVALUATOR_H_
...
inline int bottomRanksSix(int x, int n)
{
    int ret = 0;
    for (int i = 0; i < n; i++)
    {
...
inline int flipAceSix(int ranks)
{
    return
        ((ranks & ~(1 << 12)) << 1) |
        ((ranks >> 12) & 0x01);
...
inline int unflipAceSix(int ranks)
{
    return
        (ranks >> 1) |
        ((ranks & 0x01) << 12);
 ...
 */
class OmahaSixEightHandEvaluator : public PokerHandEvaluator
{
public:
    ...
    virtual PokerHandEvaluation evaluateHand(const CardSet& hand, const CardSet& board) const
    {
        ...
        // 6c2 combinations of hands cards,
        ...
        std::vector<CardSet> hand_candidates(15);
        ...
        int bmask = flipAceSix(board.rankMask() & 0x107F);
        if (nRanksTable[bmask] >= 3)
        {
            for (size_t i = 0; i < hand_candidates.size(); i++)
            {
                int hmask = flipAceSix(hand_candidates[i].rankMask() & 0x107F);
                if (nRanksTable[hmask] < 2)
                    continue;
                CardSet lowRanks(unflipAceSix(bottomRanksSix(bottomRanksSix(bmask & (~hmask), 3) | hmask, 5)));
    ...
    {
        int bmask = flipAceSix(board.rankMask() & 0x107F);
        int hmask = flipAceSix(twocard.rankMask() & 0x107F);
        if (nRanksTable[hmask] < 2)
            return PokerEvaluation();
        CardSet lowRanks(unflipAceSix(
            bottomRanksSix(bottomRanksSix(bmask & (~hmask), 3) | hmask, 5)));
    ...
    void fillHands(std::vector<CardSet>& candidates, const CardSet& cards) const
    {
        ...
        candidates[3] = (clist[0] | clist[4]);
        candidates[4] = (clist[0] | clist[5]);
        candidates[5] = (clist[1] | clist[2]);
        candidates[6] = (clist[1] | clist[3]);
        candidates[7] = (clist[1] | clist[4]);
        candidates[8] = (clist[1] | clist[5]);
        candidates[9] = (clist[2] | clist[3]);
        candidates[10] = (clist[2] | clist[4]);
        candidates[11] = (clist[2] | clist[5]);
        candidates[12] = (clist[3] | clist[4]);
        candidates[13] = (clist[3] | clist[5]);
        candidates[14] = (clist[4] | clist[5]);
    }
...
#endif  // PEVAL_OMAHASIXEIGHTHANDEVALUATOR_H_
